...
function buildScenario1Overrides(caseData) {
  ...
  if (v8Token.includes("libre")) {
    ...
    setOverride("V8", { G1: 2, G2: 4, G3: 0 }, { round: true });
  }
...
export default function Phase2RouteA({ activeRoute = "A", onSelectRoute, onGoPhase3 } = {}) {
  ...
  const canRender = !!(activeCase && activeCaseId !== "nuevo" && routeAScores);
  ...
  const pairwiseAHP = React.useMemo(() => {
  ...
  const baseScoreById = React.useMemo(() => {
    const map = new Map();
    for (const v of routeAScores?.variables || []) map.set(v.id, v);
    return map;
  }, [routeAScores]);
  ...
  const scenario2Ready = React.useMemo(() => {
    return scenario2RequiredIds.every((id) => isInputCompleteScenario2(baseScoreById.get(id)));
  }, [scenario2RequiredIds, baseScoreById, isInputCompleteScenario2]);
  const fullReady = React.useMemo(() => {
    const vars = routeAScores?.variables || [];
    if (!vars.length) return false;
    return vars.every((v) => isInputCompleteNoMaps(v));
  }, [routeAScores, isInputCompleteNoMaps]);
  ...
  const missingScenario2Labels = React.useMemo(() => {
    ...
    return vars
      ...
      .map((v) => v.label || v.id);
  }, [scenario2RequiredIds, routeAScores, isInputCompleteScenario2]);
  ...
  const getMissingModules = React.useCallback(
    ...
    [layerModuleMap, phase2RequiredIds, routeAScores]
  );
  ...
  return (
    <>
      ...
      <p style={{ marginTop: 0, opacity: 0.9 }}>
        La configuración de los Escenarios de Información representa el puente entre la fase de recolección de datos
        y el análisis avanzado. El objetivo de esta sección es permitir una transición hacia la evaluación técnica,
        ajustando el nivel de profundidad según la calidad y cantidad de los insumos disponibles.
      </p>
      ...
      <div style={{ display: activeRoute === "A" ? "block" : "none" }}>
          ...
          <div style={{ textAlign: "center" }}>
            <div style={{ marginTop: 8, display: "grid", justifyItems: "center", gap: 8 }}>
              ...
              {[
                ...
                {
                  title: "Etapa 3: Análisis de robustez (Incertidumbre + Sensibilidad)",
        ...
        <div style={{ marginTop: 10, overflowX: "auto" }}>
          ...
          <p style={{ marginTop: 10, opacity: 0.9 }}>
          Con esta estructura definida, el sistema queda habilitado para proceder a la Etapa 2, donde se determinar? la importancia relativa de cada criterio mediante el método AHP para obtener el diagnóstico definitivo.
        </p>
        </div>
        ...
        <div style={{ marginTop: 6, overflowX: "auto" }}>
          ...
          <div style={{ marginTop: 8, fontSize: 13, opacity: 0.8 }}>Fuente: Tomado de Saay, 1980</div>
        </div>
        {/*
        <div
          style={{
...
`}</style>
                ...
                <div style={{ marginTop: 14, padding: 12, borderRadius: 12, border: "1px solid rgba(245, 158, 11, 0.4)", background: "#fffbeb" }}>
                  <div style={{ fontWeight: 800, marginBottom: 6 }}>
                    &#9888; Advertencias de idoneidad
                  </div>
                </div>
                <div style={{ marginTop: 12 }}>
                  <button
        ...
        {showStage(3) ? (
          <>
            ...
            <div style={{ marginTop: 8, fontWeight: 800, fontSize: 18 }}>
              3. An&aacute;lisis de robustez (Incertidumbre + Sensibilidad)
            </div>
                  ...
                  <div style={{ marginTop: 12, overflowX: "auto" }}>
                    ...
                    {(() => {
                      ...
                      const maxScore = Math.max(
                        0,
                        ...rows.flatMap((row) => row.scores.map((score) => Number(score.total) || 0))
                      );
                      const width = Math.max(720, rows.length * 40);
                      const height = 220;
                      const pad = { top: 16, right: 16, bottom: 32, left: 36 };
                      const plotW = width - pad.left - pad.right;
                      const plotH = height - pad.top - pad.bottom;
                      const xForIndex = (i) => pad.left + (plotW * i) / Math.max(1, rows.length - 1);
                      const yForValue = (val) => pad.top + (1 - (val / (maxScore || 1))) * plotH;
                      const buildPath = (gid) =>
                        rows
                          .map((row, i) => {
                          ...
                          .join(" ");
                      const lineForGroup = (gid) => {
                        const rank = rankByGroup.get(gid) || 0;
                        return colorByRank[rank] || "#94a3b8";
                      };
                      return (
                        <div style={{ marginTop: 16 }}>
                          <div style={{ fontWeight: 800, fontSize: 14 }}>Gr&aacute;fico de l&iacute;neas m&uacute;ltiples (Line Plot)</div>
                          <div style={{ marginTop: 6, fontSize: 12, opacity: 0.75, lineHeight: 1.5 }}>
                            Eje X: peso del criterio evaluado (w<sub>test</sub>). Eje Y: score o idoneidad de cada grupo MAR
                            (Idoneidad<sub>G1</sub>, Idoneidad<sub>G2</sub>, Idoneidad<sub>G3</sub>). Series: una l&iacute;nea por cada grupo MAR.
                          </div>
                          <div style={{ marginTop: 10, overflowX: "auto" }}>
                            <svg width={width} height={height} style={{ display: "block", background: "#f8fafc", borderRadius: 12, border: "1px solid rgba(0,0,0,0.08)" }}>
                              <g stroke="rgba(0,0,0,0.08)">
                                {[0, 0.25, 0.5, 0.75, 1].map((t) => {
                                  const y = pad.top + plotH * t;
                                  return <line key={`grid-${t}`} x1={pad.left} y1={y} x2={width - pad.right} y2={y} />;
                                })}
                              </g>
                              ...
                              {["G1", "G2", "G3"].map((gid) => (
                                <path key={`line-${gid}`} d={buildPath(gid)} fill="none" stroke={lineForGroup(gid)} strokeWidth="2.5" />
                              ))}
                              {rows.map((row, i) => {
                                const x = xForIndex(i);
                                if (i % Math.ceil(rows.length / 6) !== 0 && i !== rows.length - 1) return null;
                                return (
                                  <text key={`xlab-${row.delta}-${row.wTest}`} x={x} y={height - 10} textAnchor="middle" fontSize="10" fill="#475569">
                                    {row.wTest.toFixed(2)}
                                  </text>
                                );
                              })}
                              <text x={pad.left} y={pad.top - 4} fontSize="10" fill="#475569">
                                {maxScore.toFixed(2)}
                              </text>
                              <text x={pad.left} y={height - pad.bottom + 12} fontSize="10" fill="#475569">
                                0
                              </text>
                            </svg>
                          </div>
                      ...
                      {allSameRanking ? (
                        <>
                          ...
                          <div style={{ marginTop: 6, lineHeight: 1.6 }}>
                            Este resultado sugiere que la elecci&oacute;n de {topGroup} como grupo MAR &oacute;ptimo es robusta frente a ajustes razonables en la
                            ponderaci&oacute;n del criterio {sensitivityCriterion} ({critLabel}), lo que respalda la solidez del modelo jer&aacute;rquico
                            utilizado.
                          </div>
                          <div style={{ marginTop: 6, lineHeight: 1.6 }}>
                            En otras palabras, la decisi&oacute;n no depende cr&iacute;ticamente del peso asignado a ese criterio, por lo que incluso si hubiese
                            ligeras diferencias en la percepci&oacute;n del evaluador al asignar ese peso, la conclusi&oacute;n seguir&iacute;a siendo la misma.
                          </div>
                          <div style={{ marginTop: 6, lineHeight: 1.6 }}>
                            Este tipo de comportamiento fortalece la confiabilidad del resultado obtenido en la Etapa 2, mostrando que el an&aacute;lisis AHP fue
                            coherente con la estructura del problema y los juicios de valor del usuario.
                          </div>
                        </>
                      ) : (
                        <>
      ...
      <div style={{ display: activeRoute === "B" ? "block" : "none" }}>
        <div className="panel-card" style={{ marginTop: 14 }}>
          ...
          <p style={{ opacity: 0.9 }}>
            En esta ruta, el mismo esquema conceptual se traduce a un vector de variables (V1-V35) que alimenta modelos supervisados entrenados con la base de casos.
            En lugar de solicitar pesos al usuario, la lógica se aprende a partir de ejemplos: el modelo identifica patrones que, históricamente, han conducido a G1, G2 o G3.
            Un &aacute;rbol construye la decisión con reglas del tipo si-entonces, mientras que el bosque aleatorio combina múltiples árboles para mejorar estabilidad y reducir sesgos.
          </p>
          <p style={{ opacity: 0.9 }}>
            Durante la ejecución, el sistema transforma el caso a un formato consistente (categorías y rangos definidos), estima probabilidades por grupo (p(G1), p(G2), p(G3)) y entrega una recomendación principal.
            Además, genera se&ntilde;ales de explicación (variables más influyentes) y alertas cuando faltan datos que impactan de forma relevante la decisión; en esos escenarios,
            la recomendación se reporta con menor confianza y se priorizan los campos que más reducen la solidez del resultado. Cuando se dispone de capas espaciales, la recomendación se complementa
            con mapas de idoneidad por grupo para contextualizar territorialmente la decisión.
          </p>
          <p style={{ opacity: 0.9 }}>
            <strong>Resultado esperado:</strong> recomendación basada en patrones observados + mapas de idoneidad por grupo MAR + probabilidad de ocurrencia de cada grupo (%) + estadísticos.
          </p>
        </div>
      </div>
    </div>
