...
path = Path("frontend-sigmma/src/App.jsx")
text = path.read_text(encoding="utf-8")

start = text.find("function Phase2IntroTextBlock() {")
if start == -1:
    raise SystemExit("Phase2IntroTextBlock not found")

end = text.find("function FaseAHP()", start)
if end == -1:
    raise SystemExit("FaseAHP not found")

new_block = r'''function Phase2IntroTextBlock() {
  ...
  const criteriaLabels = React.useMemo(
    () => ({
      C1: "Objetivo",
      C2: "Condiciones hidrogeológicas",
      C3: "Fuente y calidad del agua",
      C4: "Viabilidad técnica",
  ...
  const SCENARIOS = [
    {
      id: "S1",
      title: "Escenario 1 - Minimo operativo",
      subtitle: "Mayor incertidumbre (calculo final).",
      desc:
        "Se trabaja solo con los 5 criterios obligatorios marcados con (*). Permite continuar el proceso, pero los resultados son mas sensibles a faltantes.",
    },
    {
      id: "S2",
      title: "Escenario 2 - Intermedio verificado",
      subtitle: "Incertidumbre moderada (calculo final).",
      desc:
        "Incluye el Escenario 1 mas los insumos de modelos (geologico, hidrologico, hidraulico e hidrogeoquimico), tipo de acuifero y fuente/calidad.",
    },
    {
      id: "S3",
      title: "Escenario 3 - Completo robusto",
      subtitle: "Baja incertidumbre (calculo final).",
      desc:
  ...
  const GROUP_REFERENCE = [
    {
      id: "G1",
      title: "Grupo 1: Intervencion de corrientes superficiales",
      items: [
        "Objetivo",
        "Caracterizacion del acuifero (resumen o modelos)",
        "Modelos geologico, hidrologico, hidraulico e hidrogeoquimico",
        "Tipo de acuifero (libre)",
        "Estudio de la zona no saturada",
        "Porosidad y permeabilidad",
        "Fuente: superficial o escorrentia",
    ...
    },
    {
      ...
      items: [
        ...
        "Caracterizacion del acuifero (resumen o modelos)",
        "Modelos geologico, hidrologico, hidraulico e hidrogeoquimico",
        "Tipo de acuifero (libre o semiconfinado/confinado)",
    ...
    {
      id: "G3",
      title: "Grupo 3: Infiltracion superficial",
      items: [
        "Objetivo",
        "Caracterizacion del acuifero (resumen o modelos)",
        "Modelos geologico, hidrologico, hidraulico e hidrogeoquimico",
  ...
  function getAutoScore(item) {
    ...
    if (item.id === "V19" || item.id === "V20" || item.id === "V21") {
      ...
      if (item.id === "V19") {
        ...
        if (score !== null) return { score, locked: true };
        if (cap is not None) return { score: 4, locked: true };
        return { score: 0, locked: true };
      }
      if (item.id === "V20") {
        const score = mapOptionScore(conoceVol, { Si: 4, No: 0 });
        if (score !== null) return { score, locked: true };
        if (vol is not None) return { score: 4, locked: true };
        return { score: 0, locked: true };
      }
      if (item.id === "V21") {
        const score = mapOptionScore(volMenorCap, { Si: 4, No: 0 });
        if (score !== null) return { score, locked: true };
        if (cap is not None and vol is not None) return { score: 4 if vol <= cap else 0, locked: true };
        return { score: 0, locked: true };
      }
    }
  ...
  return (
    <div className="panel-card">
      {/* ... (rest of JSX omitted here for brevity) ... */}
    </div>
  );
}
'''
