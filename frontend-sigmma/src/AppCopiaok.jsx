...
function FaseInsumos({ onContinueToPhase2 } = {}) {
  ...
  /* =========================
     EXPORT PDF
  ========================= */
  function exportCaseToPDF(caseData, reportType = "general") {
    const escapeHtml = (s) =>
      String(s ?? "")
        .replaceAll("&", "&amp;")
        .replaceAll("<", "&lt;")
        ...
        .replaceAll('"', "&quot;")
        .replaceAll("'", "&#039;");
    const reportMeta = DASHBOARD_REPORT_META[reportType] || DASHBOARD_REPORT_META.general;
    const reportNote =
      typeof reportMeta.note === "function" ? reportMeta.note(caseData) : reportMeta.note || "";
    const resultsDateLabel = formatCaseResultsDateLabel(caseData);
    const caseLabel = caseData?.caseName || caseData?.nombre || caseData?.id || "sin título";
    const sanitizedCaseId = escapeHtml(caseData.id || "sin ID");
    const sanitizedReportTitle = escapeHtml(reportMeta.title);
    const reportTypeSection = `
      <div class="box" style="margin-top:8px;">
        <div class="k">Tipo de reporte</div>
        <div class="v">${escapeHtml(reportMeta.subtitle)}</div>
      </div>
    `;
    const reportNoteSection = reportNote
      ? `
        <div class="box" style="margin-top:8px;">
          <div class="k">Resumen</div>
          <div class="v">${escapeHtml(reportNote)}</div>
        </div>
      `
      : "";
    ...
    if (reportType === "rutaA") {
      ...
      if (targetId) {
        ...
        if (
          ...
          if (typeof window !== "undefined") window.pendingRouteAExportCaseId = null;
        } else {
    ...
    const html = `
      <html>
        <head>
          <title>${sanitizedReportTitle} - ${sanitizedCaseId}</title>
          <meta charset="utf-8" />
          <style>
        ...
        <body>
          ...
          <p><strong>ID:</strong> ${sanitizedCaseId} &nbsp; | &nbsp; <strong>Nombre:</strong> ${escapeHtml(caseLabel)}</p>
          <p><strong>Fecha de resultados:</strong> ${escapeHtml(resultsDateLabel)}</p>
          ${reportTypeSection}
          ${reportNoteSection}
          <div class="box">
            <div class="grid">
...
function Dashboard() {
  ...
  function exportCaseToPDFLegacy(caseData) {
    const escapeHtml = (s) =>
      String(s ?? "")
        .replaceAll("&", "&amp;")
        .replaceAll("<", "&lt;")
                        ...
                        >
                          {ROUTE_REPORT_TYPES.map((report) => (
                            <button
                              ...
                              type="button"
                              className="insumos-download-btn"
                              onClick={() => exportCaseToPDF(c, report.id)}
